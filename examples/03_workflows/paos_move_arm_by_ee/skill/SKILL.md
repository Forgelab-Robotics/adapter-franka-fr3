---
name: move-arm-by-ee-franka-fr3
description: Resolve relative end-effector motion and control the gripper opening.
metadata: {"PhyAgentOS":{"always":false,"requires":{"runtime":["move-arm-by-ee-franka-fr3"]}}}
---

# Move Franka FR3 Arm by End-Effector
...
## 6. FR3v2 profile constraints

For this bundle's `franka_fake` and `franka_real` profiles, live context must report:

- group `fr3v2_arm`;
- base/reference frame `fr3v2_link0`;
- TCP frame `fr3v2_hand_tcp`;
- Franka Hand total opening range `0.0` to `0.08` metres;
...
Starting `franka_real` only connects and reads state; it must not home or move the Hand.
Before any Action, require the operator to confirm the exact requested displacement,
clearance for the whole swept volume, Desk/FCI state, reachable external emergency stop,
and that no ROS 2/Gello controller owns the robot. Do not execute an unspecified
"test move". A cancellation is propagated to the FR3 software stop path, but software
stop is not a safety-rated emergency stop.
